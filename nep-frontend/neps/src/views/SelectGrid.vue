...
export default {
  ...
  methods: {
    ...
    changePassword() {
      ...
      this.userInfo.supervisorPwd = this.password.newPwd;
      sessionStorage.setItem("supervisor", JSON.stringify(this.userInfo));
      alert("密码修改成功");
      this.password = { newPwd: "", confirmPwd: "" };
      api.post("/supervisor/saveSupervisor", {
        supervisorId: this.userInfo.supervisorId, supervisorTel: this.userInfo.supervisorTel,
        supervisorPwd: this.password.newPwd, supervisorName: this.userInfo.supervisorName,
