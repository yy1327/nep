...
public class AqiFeedbackServiceImpl extends ServiceImpl<AqiFeedbackMapper, AqiFeedback> implements AqiFeedbackService {
    ...
    public void saveFeedback(AqiFeedback feedback) {
        String afCode = generateAfCode();
        feedback.setAfCode(afCode);
        feedback.setAfState("01");
        feedback.setCreateTime(new Date());
        save(feedback);
    }

    private String generateAfCode() {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
        String currentDate = dateFormat.format(new Date());
        String prefix = "AF" + currentDate;

        String latestCode = aqiFeedbackMapper.getLatestAfCode();

        if (latestCode != null && latestCode.startsWith(prefix)) {
            String lastSeqStr = latestCode.substring(prefix.length());
            try {
                int lastSeq = Integer.parseInt(lastSeqStr);
                int newSeq = lastSeq + 1;
                return prefix + String.format("%0" + lastSeqStr.length() + "d", newSeq);
